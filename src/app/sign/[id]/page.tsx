        ...
        try {
            ...
            const formattedSignatures = signatures.map((sig) => {
                ...
                return {
                    ...
                    page: sig.pageNumber,
                    // Use normalized coordinates if available, otherwise calculate them
                    position: {
                        x: sig.normalizedX !== undefined ? sig.normalizedX : sig.x * (sig.pdfWidth || 612) / (sig.pageWidth || 800),
                        y: sig.normalizedY !== undefined ? sig.normalizedY : sig.y * (sig.pdfHeight || 792) / (sig.pageHeight || 1100)
                    },
                    size: {
                        width: sig.normalizedWidth !== undefined ? sig.normalizedWidth : sig.width * (sig.pdfWidth || 612) / (sig.pageWidth || 800),
                        height: sig.normalizedHeight !== undefined ? sig.normalizedHeight : sig.height * (sig.pdfHeight || 792) / (sig.pageHeight || 1100)
                    },
