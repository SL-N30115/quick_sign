...
const DraggableSignature: React.FC<DraggableSignatureProps> = ({
    ...
    const convertCoordinates = useCallback((displayX: number, displayY: number, displayWidth: number, displayHeight: number) => {
        // Get current page dimensions
        const dimensions = pageDimensions.get(signature.pageNumber);
        if (!dimensions) return {x: displayX, y: displayY, width: displayWidth, height: displayHeight};

        // Get the current scale (default to 1 if not provided)
        const currentScale = dimensions.scale || 1;

        // Calculate scale ratio between PDF page at 100% and currently rendered page
        // This accounts for the current zoom level in our calculations
        const scaleX = dimensions.pdfWidth / (dimensions.width / currentScale);
        const scaleY = dimensions.pdfHeight / (dimensions.height / currentScale);

        // Store normalized position (relative to original PDF coordinates at 100% scale)
        const normalizedX = displayX * scaleX;
        const normalizedY = displayY * scaleY;
        const normalizedWidth = displayWidth * scaleX;
        const normalizedHeight = displayHeight * scaleY;
        ...
        return {
            ...
            // Add normalized values that will be used when sending to backend
            normalizedX,
            normalizedY,
            normalizedWidth,
            normalizedHeight,
    ...
    const updateSignatureInParent = useCallback(
        debounce(() => {
            ...
            const {x, y, width, height, normalizedX, normalizedY, normalizedWidth, normalizedHeight} =
                convertCoordinates(
